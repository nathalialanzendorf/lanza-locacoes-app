...
export function VeiculosListSection({ tipoFrota }: Props) {

  const basePath = veiculosBasePath(tipoFrota);

  const frotaLocacao = tipoFrota === TipoVeiculoFrota.Locacao;
  const frotaVenda = tipoFrota === TipoVeiculoFrota.Venda;
  ...
  const query = useVeiculos({ tipoFrota });
  ...
  const emptyLabel = `Nenhum veículo de ${rotuloTipoVeiculoFrota(tipoFrota).toLowerCase()} registado.`;
  ...
  return (
    ...
    <>
      ...
      <ListToolbar
        ...
        importTo={frotaLocacao ? `${basePath}/importar` : undefined}
      ...
      <DataTable
        ...
        emptyMessage={temFiltro ? "Nenhum veículo corresponde aos filtros." : emptyLabel}
        ...
        columns={[
...

          {
            ...
            sortValue: (v) =>
              ...
              frotaVenda ? rotuloStatusVeiculoVenda(v.ativo) : statusLabel(v.ativo),

            render: (v) =>

              frotaVenda ? (

                <span className={classeStatusVeiculoVenda(v.ativo)}>

                  {rotuloStatusVeiculoVenda(v.ativo)}
            ...
            ? [

                {
                  ...
                  sortValue: (v: Veiculo) =>
                    ...
                    situacaoVeiculoLabel(situacaoLocacaoVeiculo(v, placasContratoAtivo)),
